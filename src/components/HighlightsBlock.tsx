    ...
    <section className={"mt-10 rounded-[10px] bg-surface p-6 sm:p-7 max-w-[1100px] mx-auto ring-1 ring-primary/10 text-foreground relative overflow-hidden " + className}>
      ...
      {title ? (
        <h2 id="highlights-heading" className="mb-2.5 flex items-center gap-2 font-display text-[16px] font-medium leading-[1.08] tracking-normal text-primary sm:text-[17px]">
          <svg className="h-4 w-4 shrink-0 text-primary" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5">
      ...
      <ul className="mt-2 sm:mt-3 space-y-[11px] max-w-[1000px] lg:max-w-[960px] mx-auto" role="list" aria-labelledby="highlights-heading">
        {normalized.map((it, i) => (
          <li key={`${it.title}-${i}`} role="listitem" className="flex items-start gap-3">
            <span className="relative top-[9px] sm:top-[9px] inline-block h-[6px] w-[6px] sm:h-[7px] sm:w-[7px] rounded-full bg-brand flex-shrink-0" aria-hidden />
            <div>
...
/** Íconos SVG minimalistas con trazo consistente (hereda color del padre, ej. text-primary). */
const ICON_PROPS = { width: 20, height: 20, stroke: "currentColor", fill: "none", strokeWidth: 1.6 };
