    ...
    <section id={id} className={"mt-10 rounded-[10px] bg-surface p-6 sm:p-7 max-w-[1100px] mx-auto ring-1 ring-primary/10 text-foreground relative overflow-hidden " + className}>
      ...
      {title ? (
        <HeadingTag className="mb-2.5 flex items-center gap-2 font-display text-[16px] font-medium leading-[1.08] tracking-normal text-primary sm:text-[17px]">
          <svg className="h-4 w-4 shrink-0 -translate-y-[2px] text-primary" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5">
      ...
      <div className="mt-3 space-y-2.5">
        {items.map((it, idx) => (
          <details
            ...
            id={it.id}
            open={it.defaultOpen}
            className="group rounded-md border border-primary/10 bg-white/70 p-3 text-foreground transition hover:border-border-brand"
          >
            <summary className="cursor-pointer list-none text-[14px] font-medium text-primary flex items-center justify-between gap-2">
              {it.q}
              <svg className="h-3 w-3 text-primary transition-transform group-open:rotate-180" fill="none" stroke="currentColor" strokeWidth="1.5" viewBox="0 0 24 24">
                <path d="M6 9l6 6 6-6" />
              </svg>
            </summary>
